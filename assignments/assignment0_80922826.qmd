...
```
```{r}
#| label: fig-scatter-quantity-price
...
par(pin = c(3, 3))
plot(unique_sales_data$Quantity, unique_sales_data$Price, xlab="Quantity", ylab="Price", main="Relationship between Quantity and Price")
```

As can be seen in @fig-scatter-quantity-price, there appears to be a negative correlation between quantity and price. As quantity increases, price decreases. Customers appear to be more likely to purchase higher quantities of products with lower prices.
